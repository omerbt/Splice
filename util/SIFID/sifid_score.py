...
from util.SIFID.inception import InceptionV3
import torchvision
import numpy
import scipy
import pickle
...
def calculate_sifid_for_dataset(base_path, batch_size, cuda, dims, ref1, ref2):
    block_idx = InceptionV3.BLOCK_INDEX_BY_DIM[dims]
    ...
    pair_dirs = [os.path.join(base_path, d) for d in os.listdir(base_path)]
    pair_dirs = [d for d in pair_dirs if os.path.isdir(d)]

    for pair_dir in pair_dirs:
        paths_1 = [os.path.join(pair_dir, ref1, d) for d in os.listdir(os.path.join(pair_dir, ref1))]
        paths_1 = [d for d in paths_1 if d.endswith(('png', 'jpg'))]
        paths_2 = [os.path.join(pair_dir, ref2, d) for d in os.listdir(os.path.join(pair_dir, ref2))]
        paths_2 = [d for d in paths_2 if d.endswith(('png', 'jpg'))]
        m1, s1 = calculate_activation_statistics(paths_1, model, batch_size, dims, cuda)
        m2, s2 = calculate_activation_statistics(paths_2, model, batch_size, dims, cuda)
        print(pair_dir.split('/')[1], '{:.2f}'.format(float(calculate_frechet_distance(m1, s1, m2, s2))))
