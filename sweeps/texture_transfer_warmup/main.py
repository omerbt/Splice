...
from sweeps.texture_transfer_warmup.losses import LossG
...
def overfit_to_content(dataset, model):
    epochs = 1500
    lr = 0.01
    optimizer = torch.optim.Adam(model.parameters(), lr=lr)
    content_img = dataset[0]['A'].to(device)

    print("overfitting G to content...")
    for _ in range(epochs):
        optimizer.zero_grad()
        pred = model(content_img)
        loss = torch.nn.MSELoss()(pred, content_img)
        loss.backward()
        optimizer.step()
...
def train_model():
    with open("sweeps/texture_transfer_warmup/default_config.yaml", "r") as f:
