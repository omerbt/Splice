...
class LossG(torch.nn.Module):
    ...
    def __init__(self, A_img, B_img, cfg):
        ...
        self.init_lambda_config()

    def init_lambda_config(self):
        self.lambdas = dict(
            lambda_ssim=0,
            lambda_cls=self.cfg['lambda_cls'],
            lambda_patch_ssim=0,
            lambda_global_ssim=0,
            lambda_global_cls_cropped=self.cfg['lambda_global_cls_cropped'],
            lambda_global_cls=0,
            lambda_local_cls=0,
            lambda_identity=0,
            lambda_crops_cls=0
    ...
    def update_lambda_config(self):
        if self.step == self.cfg['cls_warmup']:
            self.lambdas['lambda_ssim'] = self.cfg['lambda_ssim']
            self.lambdas['lambda_patch_ssim'] = self.cfg['lambda_patch_ssim']
            self.lambdas['lambda_global_ssim'] = self.cfg['lambda_global_ssim']
            self.lambdas['loss_global_cls_cropped'] = self.cfg['lambda_global_cls_cropped']
            self.lambdas['lambda_global_cls'] = self.cfg['lambda_global_cls']
            self.lambdas['lambda_local_cls'] = self.cfg['lambda_local_cls']
            self.lambdas['lambda_identity'] = self.cfg['lambda_identity']
            self.lambdas['lambda_crops_cls'] = self.cfg['lambda_crops_cls']
    ...
    def forward(self, outputs, inputs):
        ...
        losses['loss_cls'] = self.calculate_global_cls_loss(outputs['x'], self.lambdas['lambda_cls'])
        ...
        losses['loss_global_cls'] = self.calculate_global_cls_loss(outputs['x_global'], self.lambdas['lambda_global_cls'])
        ...
        loss_G += losses['loss_global_cls_cropped'] * self.lambdas['lambda_global_cls_cropped']
        ...
        loss_G += losses['loss_crops_cls'] * self.lambdas['lambda_crops_cls']
    ...
    def calculate_crops_cls_loss(self, output):
        if self.lambdas['lambda_crops_cls'] == 0:
            return 0
        x = self.global_transform(output)
        loss = 0.0
        n_crops = 4
        for _ in range(n_crops):
            a = transforms.RandomCrop(self.lambdas['local_crops_crop_size'])(x)
            b = transforms.RandomCrop(self.lambdas['local_crops_crop_size'])(self.B)
            cls_token = self.extractor.get_feature_from_input(a)[-1][0, 0, :]
